...
configure_gnome_session() {
    ...
    # Both Ubuntu 22.04 and 24.04 ship /usr/share/gnome-session/sessions/ubuntu.session,
    # so --session=ubuntu is safe on both versions. No version detection needed.
...
# Start GNOME (--session=ubuntu works on both 22.04 and 24.04)
