...
configure_gnome_session() {
    ...
    # Choose session command based on Ubuntu version
    local session_cmd
    if [[ "$UBUNTU_VERSION" == "22.04" ]]; then
        session_cmd="exec gnome-session --session=ubuntu"
    else
        # Ubuntu 24.04: plain gnome-session (session name may differ)
        session_cmd="exec gnome-session"
    fi
