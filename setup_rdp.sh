...
configure_gnome_session() {
    ...
    # Write startwm.sh line-by-line with printf to avoid any heredoc
    # parsing ambiguity (quoting, delimiter collision, encoding issues).
    # Both Ubuntu 22.04 and 24.04 ship ubuntu.session so --session=ubuntu
    # is safe on both versions.
    printf '%s\n' \
        '#!/bin/sh' \
    ...
    # Diagnostic: confirm the key line is present right after writing.
    if grep -qF -- '--session=ubuntu' /etc/xrdp/startwm.sh; then
        log_ok "GNOME session script created (--session=ubuntu confirmed)"
    else
        log_warn "BUG: startwm.sh missing --session=ubuntu immediately after write!"
        log_warn "File content:"
